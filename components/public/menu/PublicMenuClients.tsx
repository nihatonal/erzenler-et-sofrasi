...
export function PublicMenuClient({
  ...
  // Fix: prevent SSR/client hydration mismatch for cart (Zustand is client-only)
  ...
  return (
    <main className="min-h-screen bg-brand-cream pb-24">
      ...
      {/* Cart bar — rendered only after client mount to prevent hydration mismatch */}
      {canAddToCart && mounted && cartQuantity > 0 && (
        <Link
                      ...
                      {selectedRemovables.map((item) => {
                        ...
                        return (
                          <button
                            ...
                            className={
                              selected
                                ? "rounded-full bg-brand-red border border-transparent px-3 py-2 text-sm font-semibold text-white"
